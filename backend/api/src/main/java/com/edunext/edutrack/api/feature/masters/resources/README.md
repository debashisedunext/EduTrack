# Resource Master — the list (B-010, S-07)

The read half of the Resource Master: a filterable, paginated grid of everyone
in the organisation, a bulk activate/deactivate, and an export.

Create and edit are **B-011**, reporting-manager cycle detection **B-012**,
validations **B-013**, and the bulk reassignment wizard **B-014**. None of them
are here, which is why deactivating somebody who holds open tickets stops with a
named list rather than offering to fix it.

## Shape

| File | What it is |
|---|---|
| `ResourceController` | `GET /api/v1/users`, `GET /api/v1/users/export`, `POST /api/v1/users/bulk-status` |
...
| `POST /users/bulk-status` | Admin only, like every master write. |
