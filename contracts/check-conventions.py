...
NO_PAGINATION = {
    ...
    "/onboarding/journey-templates": "OB-07 — the Module Service catalogue: every version of every service for one product, or for all of them. Bounded the way /masters/workflow-templates is — a handful of services per product — and a product's whole chain history is what the catalogue exists to show, so the retired rows are the point rather than the overflow. ObJourneyTemplateController.list computes chain-wide journey counts, dependency sets and stage groups over the full result, and both the OB-07 cards and the New Project form's service picker draw the entire set on one read: a page could not carry the chain it is grouping. Not enforced server-side. If retired versions ever pile up, the fix is a real keyset cursor plus a current-versions-only default, not a wider exemption",
