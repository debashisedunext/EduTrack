#!/usr/bin/env python3
"""
Enforce CONVENTIONS.md against openapi.yaml.

Redocly checks that the document is valid OpenAPI. This checks that it is *our*
OpenAPI — the six conventions from blueprint §13 plus the two rules that protect
row scoping and the append-only tables.

    python3 contracts/check-conventions.py          # exits non-zero on violation
    python3 contracts/check-conventions.py --list   # show current exemptions

Every exemption below is deliberate and explained in CONVENTIONS.md. Adding one
means editing this file *and* that document, which is the point: an undocumented
exception gets "fixed" by the next person, and the exception was usually right.
"""
import os
import sys

try:
    import yaml
except ImportError:
    sys.exit("pip install pyyaml")

HERE = os.path.dirname(os.path.abspath(__file__))
SPEC = os.path.join(HERE, "openapi.yaml")
VERBS = ("get", "post", "put", "patch", "delete")

# §5 — idempotent setters, or a write already guarded another way.
NO_IF_MATCH = {
    "/onboarding/journey-step-items/{itemId}/review": "OB-16 — recording one verdict, and the sibling PATCH directly below's argument applies with one extra turn of the screw. There is still no tag but the step's, from getObJourneyStep — and here the caller moves that tag themselves: a reviewer works down the check list pressing a verdict per row, so their own first press restamps the step and honouring the precondition would fail their second. The race this leaves open has no loser worth protecting: a verdict is explicitly changeable for as long as the review is open (`reviewItem` settles nothing), and the refusals that must not be raced — every row decided, the step still `PENDING_REVIEW` — are re-read inside `closeReview`'s transaction, so a stale screen cannot talk the server past them either",
    "/onboarding/journey-step-items/{itemId}": "A-118 — ticking a checklist entry. Two people ticking two DIFFERENT items on the same service is the normal case, and the only tag available is the step's, from getObJourneyStep: honouring it would fail the second person for touching a sibling item. The race this leaves open is both ticking the SAME item, which has no loser — the outcome is identical either way. The same argument as /projects/{id}/members/{userId}, where the tag would have to come from a collection with none of its own",
    "/me/password":                        "currentPassword already proves current state",
    "/portal/auth/password":               "C-121 — the portal's own version of /me/password, deliberately without currentPassword: a newly-issued client_accounts row has a password of 32 random bytes nobody knows, so there is nothing to prove by re-typing it. The CLIENT access token minted by portalRedeemCredential or an earlier portalLogin is the proof; the body names the state it wants rather than a delta, /users/{userId}/status's own idiom",
    "/users/{userId}/status":              "idempotent setter, last write wins is correct",
    "/clients/{clientId}/status":          "idempotent setter, last write wins is correct",
    "/clients/bulk-status":                "idempotent setter, like the single-client route it batches — and one If-Match cannot speak for 200 rows, while per-row tags would fail the whole batch because somebody touched one unrelated client",
    "/notifications/{notificationId}/read": "idempotent, a race is harmless",
    "/notifications/read-all":             "idempotent, a race is harmless",
    "/onboarding/notifications/{notificationId}/read": "B-112 — OB-13's half of the two routes directly above, and the same argument unchanged: idempotent, and a race is harmless. Both are scoped to the caller's own entries, so the only concurrent writer is the same person in another tab, and the loser wanted the state the winner produced. `is_read = 0` is in the UPDATE's WHERE, so the second write changes nothing at all rather than restamping `read_at`",
    "/onboarding/notifications/read-all":  "B-112 — idempotent, a race is harmless. As above, and additionally there is no tag to precondition on: the list read this would follow carries a cursor and an unread count, not a version of a set that is growing while you read it",
    "/tickets/{ticketId}/priority":        "reason mandatory and every change logged, so concurrent changes are visible not lost",
    "/tickets/{ticketId}/comments/{commentId}": "author-only inside a 5-minute window",
    "/chat/threads/{threadId}/messages/{messageId}": "author-only inside a 5-minute window",
    "/me/notification-preferences":        "your own settings only, and partial by field so two tabs cannot overwrite each other's untouched switches",
    "/projects/{projectId}/members/{userId}": "two fields on one membership row, both last-write-wins by nature; the tag would have to come from a collection read with no ETag of its own, so the precondition would be machinery around a race whose loser typed a number later and meant it",
    "/clients/{clientId}/contacts/{contactId}": "B-027 — the tag would have to come from listClientContacts, a collection with no ETag of its own, exactly as for a project member. The client this contact hangs off does have one, and a contact write moves it (contactCount and hasPrimaryContact are inside it), so the S-33 form's own precondition still catches a stale editor one level up",
    "/masters/workflow-templates/{templateId}/stages/{stageId}/deprecation": "B-042 — an idempotent setter, exactly as /users/{userId}/status and /clients/{clientId}/status are: the body names the state it wants rather than a delta, so two Admins racing produce the state whoever asked last wanted, which is the right answer rather than a lost update. Worth stating why the neighbouring writes on this screen are not exempt — updateStage replaces field values and reorderStages replaces the whole set, and in both a concurrent write silently discards what the loser could see on screen. Here there is nothing to discard. The refusals that do depend on other rows (the template's last live stage, a live stage still returning to this one) are re-read inside the transaction rather than preconditioned on a tag, so a stale screen cannot talk the server past them either",
    "/tickets/bulk-level":                 "C-017 — inherits the single-ticket route's exemption directly above: reason is mandatory and every change writes its own LEVEL_CHANGED entry, so a concurrent change is visible rather than lost. And as with /clients/bulk-status, one If-Match cannot speak for 500 rows, while per-row tags would fail the whole batch because somebody touched one unrelated ticket",
    "/onboarding/clients/{obClientId}/account": "B-126 — an idempotent setter, exactly as /users/{userId}/status is and on the same argument: the body names the state it wants rather than a delta, so two operators racing produce the state whoever asked last wanted, which is the right answer rather than a lost update. There is nothing on the account a concurrent PATCH could silently discard — it carries one settable field. Worth naming what is deliberately NOT exempt beside it: createObClientAccount and resetObClientAccountPassword both issue a credential, and both are POSTs guarded by Idempotency-Key rather than by a tag, because the thing being protected is a duplicate mail rather than a stale edit",
    "/onboarding/journey-templates/order": "C-123 — the Module Service catalogue's ↑/↓, every active template renumbered in one call. As with /clients/bulk-status and /tickets/bulk-level, one If-Match cannot speak for the whole catalogue, and per-template tags would fail the write because somebody touched an unrelated product's service. Unlike those two this genuinely is last-write-wins rather than idempotent, but the loser's only loss is a card position — nothing else on the row is touched, nothing is deleted, and the next reorder corrects it. The picker at .../{templateId}/depends-on, which does touch a single row with a real tag, is NOT exempt and requires If-Match",
}

# §5 — a detail-shaped GET with no lost-update race to precondition.
NO_ETAG = {
    "/portal/auth/credential/{token}": "A-130/C-121 — the path segment is a single-use credential token, not a resource id with a lifetime worth preconditioning. The GET changes nothing and the POST on the same path that spends the token takes no If-Match either (see NO_IF_MATCH), so there is no write this read stands in front of",
}

# §6 — bounded by a constraint the product already enforces.
NO_PAGINATION = {
    "/onboarding/journey-templates":     "C-123 — the OB-07 Module Service catalogue, and the one read that is deliberately over the WHOLE set rather than a window into it. It returns every version of every service and groups them by product in the response, which a cursor cannot express: page two of a grouping is not a grouping, and the up/down reorder the same screen owns (`PUT /onboarding/journey-templates/order`) renumbers every active template 0..N-1, so a client holding page one would reorder against positions it cannot see. Bounded in practice by the product master it is keyed to, not by a page size. Declaring a `Cursor` the server then ignores is the spec-only patch CONVENTIONS.md forbids",
    "/masters/task-types":               "11 rows",
    "/masters/priorities":               "4 rows",
    "/masters/statuses":                 "B-039 — 8 rows, and bounded by the contract's own closed StatusCode enum rather than by a habit: createStatus refuses a ninth, so a page could never contain one",
    "/masters/status-transitions":       "B-039 — the whole matrix, and it has to be the whole matrix: a cell is only meaningful beside its neighbours, and the one invariant the PUT enforces (at least one on-create row survives) is uncheckable against a page. Bounded above by statuses x statuses x roles with only the allowed cells stored — 74 rows seeded, and the S-13 grid is unreadable long before it is unpageable",
    "/masters/modules":                  "8 rows, seeded; a ninth is a row somebody adds",
    "/masters/permissions":              "18 rows, and reference data — a nineteenth arrives by migration, not by a screen",
    "/masters/roles":                    "the six of blueprint §2 plus whatever an Admin adds; the S-09 matrix is unreadable long before it is unpageable",
    "/masters/workflow-templates":       "a handful per project",
    "/masters/workflow-templates/{templateId}/mappings": "B-041 — one template's routing rules. Bounded by projects x task types, and in practice far below that: 4A.9 describes a handful of rules per template, and the two wildcard forms exist precisely so an organisation does not write one row per task type. The tab needs the whole set on one read regardless — the PUT is a whole-set replace, and a page could not carry the set it is replacing. Worth naming the ceiling that is not enforced: nothing caps this server-side, so if an organisation ever writes an exact rule for every project x task type the list grows with the product. That would be a genuine paging case, and the fix would be a real cursor rather than a wider exemption",
    "/masters/workflow-templates/{templateId}/stages": "B-040 — one template's ribbon. Eight stages on the longest of the three seeded templates, and bounded by the thing the ribbon is for: §17 already names readability at eight segments on a laptop as a risk, so a template long enough to page is a template nobody can render. The tab needs the whole set on one read regardless — canReturnTo is validated against its siblings, and reorderStages sends every id back",
    "/projects/{projectId}/sla-policies": "task types x 4 levels",
    "/clients/{clientId}/contacts":      "a short list per client",
    "/tickets/{ticketId}/attachments":   "capped at 20 per ticket",
    "/onboarding/clients/{obClientId}/attachments": "B-107 — the documents filed against one client. Bounded the way /clients/{clientId}/contacts is: a short list per client, and the card that draws it shows the whole set because a document nobody scrolled to is a signed contract nobody knows is missing. Worth naming the ceiling that is NOT enforced, because this one is an upload rather than a row somebody types: there is a per-file cap and deliberately no per-client total (the onboarding plan publishes none, and inventing one would refuse a legitimate twenty-first document), so a client accumulating hundreds of files over years would grow this list. The fix then is a real cursor, plus the per-client cap plan section 11 owes CP-04 — not a wider exemption",
    "/portal/onboarding/signoffs":        "C-122 — one client's sign-offs, pending and past together. Bounded by the journey template that produced them: one row per requiresSignoff step plus one go-live, so the ceiling is the step count of the products this client bought — a handful, on /clients/{clientId}/contacts' own reasoning. CP-05 shows pending and past as two sections of one screen, which needs the whole set on one read the way the attachments card above does",
    "/onboarding/products":              "A-118 — the catalogue of what the organisation sells, on the /masters/task-types argument. A handful of rows, read whole by the OB-04 wizard's multi-select and by OB-07's product filter, both of which need the set to render at all. Worth naming the ceiling that is not enforced: nothing caps this server-side, so an organisation selling hundreds of products would grow it — but that is a different company, and the fix would be a real cursor rather than a wider exemption",
    "/onboarding/implementation-stages": "OB-15 — the module's implementation vocabulary, on /onboarding/products' argument one master over. Six seeded rows, and OB-15 needs the whole set on one read for a reason a page control actively breaks: sequence is a position, and a position is only meaningful beside its neighbours, so an admin moving a row to 3 while looking at page 2 would be moving it somewhere they cannot see. Worth naming the ceiling that is not enforced: nothing caps this server-side, so an organisation that kept adding stages would grow it — but a list of implementation stages long enough to page is a picker nobody can use, and the fix then is a shorter vocabulary rather than a wider exemption",
    "/notifications/pending":            "a queue drained by acknowledging, not paged; a cursor would outlive the rows it points past",
    "/me/notification-preferences":      "one row per NotificationEvent — 25, and bounded by the enum",
    "/tickets/{ticketId}/status-requests": "open requests on one ticket, and at most one per manager who can ask about it",
    "/me/awaiting-response":             "your own unanswered asks; bounded server-side and ordered longest-wait-first, which a cursor would fight",
    "/chat/ticket-cards":                "bounded by the caller's own `codes` list, and capped below that; paging a lookup you supplied the keys for is not a page",
    "/projects/{projectId}/members":     "one project's team — tens of people, and the S-10 tab totals their allocations, so it needs the whole set on every read anyway",
    "/imports/{schema}/mapping-presets": "B-033 — one row per column mapping an Admin chose to name and reuse. Bounded by the only thing that creates them, which is a person typing a name on S-34 step 3: an organisation has one or two exports it imports from, not a hundred. The step-3 control is a picker, so it needs every preset on the one read anyway",
    "/import-batches":                   "B-037 — capped server-side at 50 and the cap is on the response as `limit`, so a client can tell \"the recent ones\" from \"all of them\". A cursor over a panel nobody scrolls would be Cursor plumbing for a list whose useful contents are the last few runs; the reason to reach it at all is to find the import you have just got wrong. Listed here rather than left to pass silently on `data` being an object, for the reason /reports below gives: the exemption is the decision, and it should be reviewable if the shape ever changes",
    "/onboarding/prereq-template":       "A-118 — one version of the org-wide prerequisites master, read whole. Both callers need the complete set to do anything with it: createObClient snapshots it, and OB-14 draws it. A partial answer would let a client be boarded against half a checklist — which is not a slow screen, it is a gate that opens because nobody fetched page two. The set is deliberately short (plan §14 names a short mandatory list as the mitigation for the gate stalling every journey), so the bound is a product decision that is actively defended rather than a habit. Listed here rather than left to pass silently on `data` being an object, for the reason /reports gives: the exemption is the decision",
    "/onboarding/clients/{obClientId}/prereqs": "A-118 — one client's snapshot of the above, so it inherits that bound exactly. And the gate arithmetic is stated over the whole set: mandatoryVerified/mandatoryTotal and optionalOutstanding are sums over `tasks`, so a page-two task nobody fetched is a mandatory task nobody knows is outstanding, on the one screen whose job is to say what is outstanding",
    "/onboarding/dashboard/summary":     "A-118 — seven cards, and the closed ObDashboardCardKey enum is the bound: no caller can ask for an eighth, and OB-02's layout draws all seven at once. The /dashboard/widgets argument, arriving at the same place from the other direction — there the client supplies the keys, here the contract fixes them, and in both cases a cursor would page a set that is already complete. This exemption can only stop being true by somebody adding a card, which is a contract change and gets read",
    "/onboarding/notification-templates": "A-118 — one row per (eventCode, channel) over plan §7's event list, which is bounded by the code that declares it: an event arrives with a producer, in a release, not by use. /masters/notification-templates' argument on the module's own set, and OB-12 groups the whole set by category on one screen — an admin comparing the email and in-app wording of the same event needs both rows visible at once, which a cursor could split across pages",
    "/onboarding/reports":               "A-118 — the module's report catalogue, on /reports' argument unchanged: a list of what the deployment can run, not a query over rows, bounded by the code that declares it. Twelve today (plan §10), of which five are declared unavailable pending the §11.6 call, and a thirteenth arrives by a release rather than by use. OB-10 groups the whole set by category on one screen. Listed rather than left to pass silently on `data` being an object: the exemption is the decision, and it should be reviewable if the shape ever changes",
    "/reports":                          "the eighteen of §7.8 — a catalogue of what the deployment can run, not a query over rows. It is bounded by the code that declares it, so a nineteenth arrives by a release rather than by use, and the hub groups the whole set by category on one screen. Listed here rather than left to pass silently on `data` being an object: the exemption is the decision, and it should be reviewable if the shape ever changes",
    "/masters/notification-templates":   "at most one row per NotificationEvent per channel — 27 x 3, so bounded by two enums the way /me/notification-preferences is bounded by one; the S-15 grid groups by event and needs every channel of a group together, which a cursor cutting mid-event would break",
    "/reports/schedules":                "A-065 — your own scheduled reports, cancelled ones included so \"why did this stop arriving\" has an answer. Bounded the way /me/awaiting-response is: one person against eighteen catalogued reports (/reports) x three cadences is the practical ceiling on how many standing instructions somebody keeps, and S-24's screen is a management list rather than a query over rows — it needs the whole set on one read to show a schedule beside its own recent runs. Unlike /import-batches this has no server-side cap standing behind the exemption, so if cancel-and-recreate churn ever makes the list genuinely long, the fix is Stream A's — either a cap like import-batches' 50, or real cursor pagination, not a spec-only patch here",
    "/dashboard/widgets":                "A-073 — the same argument /chat/ticket-cards makes, and here it is exact: the caller supplies the keys, so paging a lookup you provided the keys for is not a page. Bounded twice over — by the request (a client asks for the widgets it is about to draw, ten on the fullest dashboard and two on a Developer's) and above that by the contract's own closed 14-value enum on `keys`, so no request can name a fifteenth. A cursor would page a set the client already enumerated, and fetching a second page would be a second round trip on the one endpoint whose entire purpose is removing round trips from S-05's first paint. Unlike /reports/schedules the ceiling is not a habit but the enum, so this exemption cannot quietly stop being true: the way it would fail is somebody adding a fifteenth widget key, which is a contract change and gets read",
}

# §8 — GET only. No mutation verb, ever.
APPEND_ONLY = ("/history", "/effort-logs", "/audit-logs")

# §7 — 403 is legitimate only where the failure does not depend on a row.
#
# Held as a dict rather than a set since B-015, because the violation message
# already says "add it to ROWLESS_403 with a reason" and there was nowhere to
# put one. Membership tests read the same either way.
ROWLESS_403 = {
    "/onboarding/projects/{obProjectId}": "OB-15 — the PATCH and the DELETE on a project header. The two module roles this refuses, OB Viewer and OB Step Owner, can *reach* the route: A-112's scope let them read the project on the workspace, so a 404 would deny a row they are looking at. This is /onboarding/clients/{obClientId}'s entry (B-103) one promotion up, on the table that replaced it. Out-of-scope projects are still 404 and provably so — `delete` runs the scoped `findDetail` BEFORE it asks `scope.mayWrite()`, so a project this caller cannot see never reaches the capability check at all, and `update` decides the capability from the scope before it loads any row. Worth separating the other refusals these two routes make, because neither is here: `COMPLETED` is 422 (`ob-project-status-not-earned`) because it depends on the project earning it, and a delete blocked by one of the nine tables pointing at its steps is 409, because that depends on what happened to the row rather than on the caller",
    "/onboarding/journey-steps/{stepId}/checklist/submit": "OB-16 — the task's Send for verification, the bulk sibling of /journey-step-items/{itemId}/submit. Gated by `requireOwnership`, and the caller reaching it has already rendered this step through the unscoped `getObJourneyStep` the OB-06 panel draws — so a 404 would deny a task they are looking at, and the 403 conceals no existence. What it refuses is the capability of being this task's owner (or its backup, or the project implementor it falls through to), which is decided before a single row of the check list is read. Out-of-scope steps are still 404 — that is the scope guard, not this gate. The refusals that depend on the rows are NOT here and are 422: an unanswered row, a list already out, and a closed task",
    "/onboarding/journey-steps/{stepId}/review/verdict": "OB-16 — one verdict for the whole check list, gated by `requireReviewer`, so /journey-steps/{stepId}/review/complete's entry applies unchanged and in the same order: a caller with no onboarding standing at all is answered 404 by `requireModuleStanding`, so the route confirms no id to anybody outside the module, and only a caller who can already read this step sees the 403. Gated by role rather than by ownership for the reason the per-row /review entry gives: reviewing is by definition an act on somebody else's work, so an ownership check would refuse exactly the caller the route exists for. The row-dependent refusals are 422 — the task not being PENDING_REVIEW, a verdict of NOT_REVIEWED, a rejection with no reason, and nothing being out to decide on",
    "/onboarding/journey-steps/{stepId}/review/complete": "OB-16 — closing an accepted manager review. `requireReviewer` is skipObJourneyStep's argument on the review gate and splits its two refusals the same way, in this order: a caller with no onboarding standing at all is answered 404 by `requireModuleStanding` — so the route confirms no id to anybody outside the module — and only a caller who already holds an onboarding role, and can therefore already read this step through the unscoped `getObJourneyStep` the OB-06 panel renders, can see the 403. So the refusal conceals no existence; what it refuses is the capability of being this project's `implementorManagerUserId` or an `OB_ADMIN`. The refusals that depend on the row are NOT here and are 422: the step not being `PENDING_REVIEW`, and a row still unreviewed",
    "/onboarding/journey-step-items/{itemId}/review": "OB-16 — one verdict on one check-list row, gated by `requireReviewer`, so the entry above applies unchanged: 404 for a caller outside the module, 403 only for one who can already read the step this row hangs off. Gated by role rather than by ownership on purpose — reviewing is by definition an act on somebody else's work, so an ownership check would refuse exactly the caller the route exists for. The row-dependent refusals are 422 (`StepItemAlreadyVerifiedException`, and a row that is not on the reviewer's desk)",
    "/onboarding/journey-step-items/{itemId}/send-back": "OB-16 — the reviewer's per-row Send, `requireReviewer` again and the same split. As above: the module gate answers 404 first, so the 403 is only ever seen by a caller who can already read the step",
    "/onboarding/journey-steps/{stepId}/skip": "A-118 — skipping is Manager/Admin only, and the caller reaching this route has already read the step through getObJourneyStep, which A-112's scope resolver let them see. So the refusal cannot be concealing an existence: a 404 here would deny a row the same caller just rendered in the OB-06 panel. The capability is decided before the step id is resolved, exactly as /import-batches/{batchId}/reverse argues for the one route that deletes client rows. Out-of-scope steps are still 404 — that is the scope guard, not this gate",
    "/onboarding/prereq-tasks/{prereqTaskId}/skip": "A-118 — the waiver in plan §5.3, Manager/Admin only, and it inherits skipObJourneyStep's argument one table over: the caller reaching this route has already read the task through getObClientPrereqTask, so a 404 would deny a row they are looking at on OB-05. The capability is decided from the role before the task id is resolved. Worth separating the two refusals this route can make, because only one of them is here: WHO may waive is the 403; WHETHER this particular task may be waived at all is 422 (ob-prereq-mandatory-not-skippable), because that depends on the row's own isMandatory rather than on the caller — the same split C-033 records between deleteComment's 403 and editComment's 422. Out-of-scope tasks are still 404",
    "/onboarding/clients/{obClientId}": "B-103 closing the gap B-102 raised on updateObClient. The PATCH names three writers — OB Admin, Onboarding Manager, and Sales for a client they created — and declared no status for breaking the rule. The two roles it excludes, OB Viewer and OB Step Owner, can *reach* the route: A-112's scope let them read the client on OB-05, so a 404 would deny a row they are looking at. The capability is decided from the module role before anything about this client is considered, which is skipObJourneyStep's argument on a different verb. Out-of-scope clients are still 404 — that is the scope guard, not this gate, and the write service runs the scoped read first so the two cannot be confused",
    "/onboarding/clients/{obClientId}/contacts": "B-103 — the SPOC panel inherits the rule of the client it hangs off, and therefore the argument above unchanged. A Viewer reaching this POST has already rendered the client's contacts through getObClient, so the refusal conceals no existence; what it refuses is a capability decided from the module role before the body is read. A contact under a client this caller cannot see never reaches the check at all — the scoped client read comes first and answers 404",
    "/onboarding/clients/{obClientId}/contacts/{contactId}": "B-103 — as above, for the PATCH and the DELETE. Worth separating the two refusals these routes can make, because only one of them is here: WHO may edit a SPOC is the 403; WHETHER this particular SPOC may be demoted or deactivated is 409 (ob-contact-primary-required), because that depends on the row being the client's last primary rather than on the caller. The same split C-033 records between deleteComment's 403 and editComment's 422",
    "/onboarding/module-access": "A-117 — organisation-wide administration, not a row-scoped resource. The caller has already passed ModuleGuard to reach this route at all, so the module's existence is not what is being concealed, and there is no grant whose existence a 404 could protect: the list is over every user in the organisation and the create has no row yet. /masters/roles' argument, on the table that decides who may reach a module",
    "/onboarding/module-access/{grantId}/revoke": "A-117 — as above, and the row here does exist. The refusal still does not depend on it: OB Admin is decided from the module role before the grant id is looked up, so a Manager cannot distinguish a real grant id from an invented one either way. The same argument /imports/{schema}/mapping-presets/{presetId} records. Note the second refusal this route can make is NOT here — revoking the last live OB_ADMIN grant answers 422, because that depends on the state of the table rather than on the caller",
    "/onboarding/settings": "A-118 — org-wide settings are not a row. OB Admin is a capability the caller either holds or does not, decided before anything is read, and the resource has no existence a 404 could conceal. Admin-only on the read too, so a 403 tells a non-Admin exactly what ModuleGuard's 404 already did. The same argument /attachments/limits makes one module over",
    "/onboarding/notification-templates": "A-118 — master data, not row-scoped, and Admin-only on every verb including the read. So a 403 tells a non-Admin nothing they could not already infer: nobody without the OB Admin role learns that any template exists, whichever status we answer. /masters/notification-templates' argument, unchanged",
    "/onboarding/notification-templates/vocabulary": "A-118 — same: a catalogue of enum values, Admin-only because the screen it feeds is",
    "/onboarding/notification-templates/{templateId}": "A-118 — same: not row-scoped. Every read here is Admin-only, so a 403 concedes only what a 401 would have",
    "/onboarding/clients/{obClientId}/applications": "B-104 — the purchases panel inherits the rule of the client it hangs off, so this is the contacts entry above unchanged. A Viewer reaching this POST has already rendered the client's purchases and journey strips through getObClient, so the refusal conceals no existence; what it refuses is a capability decided from the module role before the body is read. A purchase under a client this caller cannot see never reaches the check — the scoped client read comes first and answers 404",
    "/onboarding/clients/{obClientId}/applications/{applicationId}": "B-104 — as above, for the PATCH. The same split the contacts entry records applies here and is worth naming with its own pair: WHO may edit a purchase is the 403; WHETHER this particular purchase may be repointed at another product is 409 (ob-application-product-immutable), because that depends on the row already having a journey pinned to its product rather than on the caller",
    "/onboarding/clients/{obClientId}/attachments": "B-107 — the documents card hangs off the client exactly as the SPOC, purchases and requirements panels do, so it inherits their entries unchanged. A Viewer reaching this POST has already rendered the client through getObClient; what the 403 refuses is a capability decided from the module role before a single byte of the body is read, which is also the point of refusing there rather than after sniffing a 10 MB file. An attachment under a client this caller cannot see never reaches the check at all — the scoped client read comes first and answers 404",
    "/onboarding/clients/{obClientId}/attachments/{attachmentId}": "B-107 — as above, for the DELETE, and here the row does exist and the caller is looking at it: they fetched the listing it is in a moment ago, so a 404 would deny a file rendered on their own screen. Worth separating the two things this route decides, because only one of them is a status: WHO may remove a document is this 403 (the uploader, an OB Admin, or an Onboarding Manager); whether the removal leaves a visible tombstone is not a refusal at all but a read-time consequence of who removed it and when. Out-of-scope clients are still 404 — that is the scope guard, not this gate",
    "/onboarding/clients/{obClientId}/requirements": "B-106 — the requirements list hangs off the client exactly as the SPOC and purchases panels do, so it inherits their entry above unchanged. A Viewer reaching this POST has already rendered the client's requirements through getObClient; what the 403 refuses is a capability decided from the module role before the body is read, and a requirement under a client this caller cannot see never reaches the check at all — the scoped client read comes first and answers 404",
    "/onboarding/clients/{obClientId}/requirements/{requirementId}": "B-106 — as above, for the PATCH and the DELETE. Same split the contacts entry records: WHO may edit a requirement is the 403; whether the body survives §3.9's sanitiser is a 400, because that depends on what was sent rather than on the caller",
    "/audit-logs":                        "Admin-only regardless of what is in it",
    "/masters/roles":                     "Admin-only screen; there is no row yet on a create",
    "/masters/workflow-templates": "B-041 — master data, not row-scoped, and there is no row yet on a create. The collection read is open to all six roles because every ticket page renders a ribbon that names its template, so a 403 on the POST concedes only what the GET already told the same caller: which templates exist. Admin-only is about authoring the workflow, not about seeing it — B-039's argument for the transition matrix, one table over",
    "/masters/workflow-templates/{templateId}": "B-041 — master data, not row-scoped. Every template is already public through listWorkflowTemplates, which all six roles read because a ribbon renders on every ticket page and names its template — so a 404 on the PATCH or DELETE would conceal nothing and only make a permission failure look like a missing row. And there is no assignee, project or client on a workflow template for ScopeResolver to answer about",
    "/masters/workflow-templates/{templateId}/mappings": "B-041 — as above, and the rules are readable by all six roles for the same reason the template is. The PUT is Admin-only because authoring the routing policy is, not because the policy is a secret: the GET already tells the same caller which project and task type route where. That is B-039's argument for the transition matrix, on the table that decides which ribbon a ticket gets rather than which moves it may make",
    "/masters/workflow-templates/{templateId}/stages": "B-040 — master data is not row-scoped, and every stage is already public through listStages. The create needs master.write; refusing it with 404 would deny the existence of a template the same caller may read",
    "/masters/workflow-templates/{templateId}/stages/{stageId}": "B-040 — as above. The stage is readable by all six roles, so a 404 on the PATCH would conceal nothing and only make a permission failure look like a missing row",
    "/masters/workflow-templates/{templateId}/stages/{stageId}/deprecation": "B-042 — as the two stage routes above. The stage is readable by all six roles through listStages, so a 404 on the setter would conceal nothing and only make a permission failure look like a missing row. And it is master data rather than a scoped row: there is no assignee, project or client on a workflow stage for ScopeResolver to answer about",
    "/masters/workflow-templates/{templateId}/stages/order": "B-040 — as above, and the reorder has no row of its own to hide behind",
    "/masters/roles/{roleId}":            "master data, not row-scoped — every role is already public through listRoles, so a 403 leaks nothing a 404 would hide",
    "/masters/roles/{roleId}/permissions": "same: the role's existence is public, only the write is Admin-only",
    "/masters/priorities":                 "master data, not row-scoped — every active level is already public through listPriorities, and there is no row yet on a create",
    "/masters/priorities/{priorityId}":    "same: the level's existence is public, only the write is Admin-only",
    "/masters/statuses":                   "B-039 — master data, not row-scoped. Every active status is already public through listStatuses, which every role reads to render a ticket's own status chip, and there is no row yet on a create",
    "/masters/statuses/{statusId}":        "same: the status's existence is public through that list, so only the write is Admin-only and a 403 conceals nothing a 404 would have",
    "/masters/status-transitions":         "B-039 — the matrix is not a row either, and the read is open to all six roles because a ticket page has to know which moves to offer. So a 403 on the PUT concedes only what the GET already told the same caller: which moves exist. Admin-only is about authoring the policy, not about seeing it",
    "/masters/task-types":                 "master data, not row-scoped — every task type is already public through listTaskTypes, and there is no row yet on a create",
    "/masters/task-types/{taskTypeId}":    "same: the type's existence is public, only the write is Admin-only",
    "/masters/notification-templates":     "Admin-only screen, reads included — master data, not row-scoped, so there is no row whose existence a 404 could protect",
    "/masters/notification-templates/vocabulary": "same: a catalogue of enum values, Admin-only because the screen it feeds is",
    "/masters/notification-templates/{templateId}": "same: not row-scoped. Every read here is Admin-only, so a 403 tells a non-Admin only what the 401 already did",
    "/attachments/limits":                 "org-wide settings, not a row — `master.write` is a capability the caller either holds or does not, and the resource has no existence a 404 could conceal (every role already reads it)",
    "/clients/bulk-status":                "master data, not row-scoped — the ids in the body name rows listClients already returned, and §4B.2's ticket dropdown makes that readable by all six roles, so a 403 conceals nothing a 404 would have",
    "/clients":                            "master data, not row-scoped — every client is already public through listClients, which §4B.2's ticket dropdown makes readable by all six roles, and there is no row yet on a create",
    "/clients/{clientId}":                 "same: the client's existence is public through the same list, so only the write is Admin-only and a 403 on it conceals nothing a 404 would have",
    "/clients/{clientId}/contacts":         "B-027 — master data, not row-scoped. Every contact of every client is already public through listClientContacts, which §4B.2's reporter dropdown makes readable by all six roles, and there is no row yet on a create",
    "/clients/{clientId}/contacts/{contactId}": "same: the contact's existence is public through that list, so only the write is Admin-only. A contact under a *different* client is still 404 — that is the nesting, not the capability",
    "/imports/{schema}/upload":            "B-032 — an upload is not a row either, and at this point it is not even the caller's data in our tables: the file is theirs, staged and unwritten. `master.write` is a capability the caller holds or does not, decided before the multipart body is looked at, so the refusal cannot depend on anything a 404 would be concealing",
    "/imports/{schema}/fields":            "B-033 — a schema's column list is not a row at all: it is the shape of a Java class, the same declarations the template route already hands out as a workbook and the upload route already reads to auto-match. Nothing about the organisation is in it, so there is no existence a 404 could protect, and the refusal is about the screen for the reason recorded on the template route",
    "/imports/{schema}/mapping-presets":   "B-033 — org-wide master data, and Admin-only on every verb including the read. So a 403 tells a non-Admin exactly what the 401 already did: nobody without master.write can learn that any preset exists, whichever status we answer. The same argument as /masters/notification-templates",
    "/imports/{schema}/mapping-presets/{presetId}": "same, and worth being explicit that the row here does exist. The refusal still does not depend on it — master.write is decided before the id is looked up, so a Developer cannot distinguish a real preset id from an invented one either way",
    "/imports/{schema}/commit":            "B-035 — the one route on this path that genuinely is a bulk write to the client master, which makes master.write the capability by any reading. Rowless for the same reason the validate route above is: the refusal is decided before the uploadId in the body is resolved, so it cannot depend on anything a 404 would be concealing, and the batch row it would create does not exist yet",
    "/import-batches/{batchId}":           "B-035 — the row here does exist, and the refusal still does not depend on it: master.write is decided before the id is looked up, so a Developer cannot distinguish a real batch id from an invented one either way. The same argument as /imports/{schema}/mapping-presets/{presetId}, and an import batch is org-wide master data rather than a scoped row — there is no assignee, project or client on it for ScopeResolver to answer about",
    "/import-batches":                    "B-037 — a list is not a row. The refusal is decided before any batch is read, so it conceals nothing a 404 would have, and an import batch is org-wide master data with no assignee, project or client on it for ScopeResolver to answer about. The panel it feeds is on S-34, which §7.4 puts inside the Admin-only master-data module",
    "/import-batches/{batchId}/reverse":   "B-037 — inherits the poll's argument: master.write is decided before the batch id is looked up, so a Developer cannot tell a real batch from an invented one either way. Worth stating that this is the one route in the product that DELETES client rows, and that it still does not deserve a 404: concealing the batch's existence from a caller who cannot reverse it protects nothing, because listImportBatches and getImportBatch already show it to everyone holding the same capability",
    "/import-batches/{batchId}/error-report": "B-036 — inherits the poll's argument directly above, and adds nothing to it: master.write is decided before the batch id is looked up, so a Developer cannot tell a real batch from an invented one either way. The file itself is org-wide master data (an extract of the client master, which listClients already makes readable by all six roles) rather than a scoped row, so there is no assignee, project or client on it for ScopeResolver to answer about. Worth noting the 404s here are about something else entirely — no such batch, or no report on it — and neither of those is concealing an existence from a caller who lacks the capability",
    "/imports/{schema}/validate":          "B-034 — the dry run reads a file the caller staged themselves and writes nothing, so there is no row on either side of it for a 404 to protect. `master.write` is decided before the uploadId in the body is looked up, which is also why a 403 here cannot be confused with the 422 an expired upload answers: one is about the caller, the other about their own staging entry",
    "/users/{userId}/timesheet/approval":  "B-065 — only the role gate answers 403. `hasAnyRole('ADMIN','PM')` refuses Support and the three delivery roles before the named user is looked up at all, so that refusal cannot depend on which resource is in the path. The row question underneath it — Admin or this resource's own direct manager, nobody else — is NOT rowless and does not appear here: it answers 404, on GET .../timesheet's own precedent, so an id cannot be walked to learn who reports to whom",
    "/imports/{schema}/template":          "B-031 — a blank template is not a row. It carries no organisation data at all: column headings, the enum values already public through the client list's own filters, and a fictional example row. There is nothing whose existence a 404 could protect, and the refusal is about the screen (S-34 is inside §7.4's Admin-only master data module) rather than about anything in the file",
    # C-028, and the only entry here that IS row-scoped — worth reading before
    # it is used as a precedent.
    #
    # Every other line above says "not a row, so there is no existence to
    # protect". This one concedes the row exists and refuses the verb anyway,
    # because by the time §4B.4's deletion rule is consulted the caller has
    # already passed ScopedTickets AND the attachment has been confirmed to be on
    # that ticket. They are looking at it in a listing they just fetched, with its
    # file name and its size. The 404 would be protecting an existence the
    # previous request disclosed.
    #
    # It would also be actively worse: the client would have to render "that
    # attachment no longer exists" for a file still on screen, and the user's next
    # move — asking a PM to remove it — is one the true answer suggests and the
    # false one hides.
    #
    # The line this preserves: 404 conceals whether a row exists, 403 concedes
    # that it does and refuses the verb. An out-of-scope ticket is still 404 here,
    # thrown before this refusal is reachable.
    "/tickets/{ticketId}/attachments/{attachmentId}": "row-scoped, but scope has already answered — a 404 here would deny a file the caller can see in the listing they just fetched; §4B.4's rule is about who may remove it, not whether it exists",
    # C-033, and now the SECOND row-scoped entry. Read the C-028 block above
    # first; the argument is identical and is not restated here.
    #
    # One difference worth recording, because it is why this feature has a 403 on
    # one verb and not the other. `deleteComment` is refused purely by who is
    # asking — there is no window on a comment deletion at all — which is what 403
    # means. `editComment` is refused by the state of the row: the five minutes
    # have run out, and the caller was entitled to it four minutes ago. That is a
    # 422, it is what the contract has said since D-001, and it needs no exemption
    # here.
    "/tickets/{ticketId}/comments/{commentId}": "row-scoped, but scope has already answered — the caller is looking at the comment in a thread they just fetched, with its author and its text; §4B.5's rule is about who may remove it, not whether it exists. The DELETE is the 403; the PATCH answers 422 because what refuses it is the row's age rather than the caller",
    # C-017, and back to the ordinary argument rather than the two row-scoped
    # concessions above: **no ticket id has been looked at when this fires.**
    #
    # §7.5 puts the three bulk actions behind PM and Admin as a *capability*.
    # The refusal is decided from the caller's role alone, before the body is
    # read, so there is nothing about which tickets exist for a 404 to conceal —
    # a Developer gets the identical 403 whether the body names fifty real
    # tickets or fifty invented ones.
    #
    # Row scoping is untouched and still answers 404 in its own way: a PM whose
    # selection reaches past their projects gets those ids back as per-ticket
    # `Not found or out of scope` in the 200 result, deliberately
    # indistinguishable from ids that were never there. The capability is the
    # 403; the rows are still the 404.
    "/tickets/bulk-level":                 "PM/Admin capability, decided from the role before any ticket id is read — a Developer sees the same 403 whether the body names real tickets or invented ones. Out-of-scope ids are still concealed, as per-ticket 'Not found or out of scope' inside the 200",
    "/tickets/bulk-close":                 "same",
}


def main():
    spec = yaml.safe_load(open(SPEC, encoding="utf-8"))
    schemas = spec["components"]["schemas"]
    paths = spec["paths"]
    fail = []

    def ops():
        for p, item in paths.items():
            for m, o in item.items():
                if m in VERBS:
                    yield p, m, o

    def params(p, o):
        return str(o.get("parameters", [])) + str(paths[p].get("parameters", []))

    def body_schema(o, code):
        c = o.get("responses", {}).get(code, {}).get("content", {})
        return c.get("application/json", {}).get("schema", {})

    def resolved(schema):
        """
        Follow a `$ref`, or hand back an inline schema unchanged.

        The second half was missing until A-029, and its absence was a **false
        positive that failed somebody else's build**. Every operation until then
        answered with a named component, so this was only ever handed a `$ref`;
        the first inline 2xx body resolved to `{}`, reported no `data` property,
        and the §2 envelope rule fired on a response that was correctly wrapped.
        Its author then went looking for a defect that was never there.

        A checker that is wrong in the strict direction is worse than no
        checker: it spends other people's time and teaches them to distrust it.
        """
        ref = schema.get("$ref") if isinstance(schema, dict) else None
        if not ref:
            return schema if isinstance(schema, dict) else {}
        return schemas.get(ref.split("/")[-1], {})

    if "--list" in sys.argv:
        print("If-Match exemptions:")
        for k, v in NO_IF_MATCH.items():
            print("  %-46s %s" % (k, v))
        print("\nPagination exemptions:")
        for k, v in NO_PAGINATION.items():
            print("  %-46s %s" % (k, v))
        print("\nRowless 403 exemptions:")
        for k, v in ROWLESS_403.items():
            print("  %-46s %s" % (k, v))
        return 0

    for p, m, o in ops():
        opid = o.get("operationId", "%s %s" % (m.upper(), p))

        # §2 — every 2xx JSON body wrapped in { data }
        for code, r in o.get("responses", {}).items():
            if not str(code).startswith("2"):
                continue
            sch = (r.get("content") or {}).get("application/json", {}).get("schema")
            if sch and "data" not in (resolved(sch).get("properties") or {}):
                fail.append("%s: 2xx body is not wrapped in { data }" % opid)

        # §3 — errors are problem+json
        for code, r in o.get("responses", {}).items():
            if str(code)[0] in "45":
                types = set((r.get("content") or {}).keys())
                if types and types != {"application/problem+json"}:
                    fail.append("%s: %s uses %s, not application/problem+json"
                                % (opid, code, sorted(types)))

        # §4 — creates take Idempotency-Key
        if m == "post" and "201" in o.get("responses", {}) \
                and "IdempotencyKey" not in params(p, o):
            fail.append("%s: create without Idempotency-Key" % opid)

        # §5 — risky writes take If-Match and can answer 412
        if m in ("patch", "put") and p not in NO_IF_MATCH:
            if "IfMatch" not in params(p, o):
                fail.append("%s: write without If-Match (add an exemption with a reason "
                            "if a lost update genuinely does not matter here)" % opid)
            elif "412" not in o.get("responses", {}):
                fail.append("%s: takes If-Match but never answers 412" % opid)

        # §5 — detail reads carry ETag
        if m == "get" and (p.rstrip("/").endswith("}") or p.endswith("/full")) and p not in NO_ETAG:
            if "ETag" not in str(o.get("responses", {}).get("200", {})):
                fail.append("%s: detail read without ETag" % opid)

        # §6 — collections paginate
        if m == "get" and p not in NO_PAGINATION:
            data = (resolved(body_schema(o, "200")).get("properties") or {}).get("data", {})
            if data.get("type") == "array":
                if "Cursor" not in params(p, o):
                    fail.append("%s: collection without cursor pagination" % opid)
                if "meta" not in (resolved(body_schema(o, "200")).get("properties") or {}):
                    fail.append("%s: paginated collection without meta" % opid)

        # §7 — 403 only where the failure does not depend on a row
        if "403" in o.get("responses", {}) and p not in ROWLESS_403:
            fail.append("%s: declares 403 — out-of-scope rows must return 404, "
                        "or add it to ROWLESS_403 with a reason" % opid)

        # codegen names every client function from this
        if not o.get("operationId"):
            fail.append("%s %s: no operationId" % (m.upper(), p))

    # §8 — append-only paths expose no mutation verb
    for p, item in paths.items():
        if any(a in p for a in APPEND_ONLY):
            bad = sorted(set(item) & {"put", "patch", "delete", "post"} - {"post"} )
            if p.endswith("/history") or p.endswith("/effort-logs") or p.endswith("/audit-logs"):
                bad = sorted(set(item) & {"put", "patch", "delete", "post"})
            if bad:
                fail.append("%s: %s on an append-only path. A correction is a new "
                            "compensating entry, not a mutation." % (p, bad))

    ops_count = sum(1 for _ in ops())
    if fail:
        print("CONVENTIONS: %d violation(s) across %d operations\n" % (len(fail), ops_count))
        for f in fail:
            print("  ✗ " + f)
        print("\nSee contracts/CONVENTIONS.md")
        return 1
    print("CONVENTIONS: %d operations, %d paths — all conform." % (ops_count, len(paths)))
    print("  exemptions: %d If-Match, %d pagination (see --list)"
          % (len(NO_IF_MATCH), len(NO_PAGINATION)))
    return 0


if __name__ == "__main__":
    sys.exit(main())
