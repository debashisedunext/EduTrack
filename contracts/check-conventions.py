...
ROWLESS_403 = {
    ...
    "/onboarding/projects/{obProjectId}": "the project header inherits the client's entry (B-103) one level down, and ObProjectWriteService already draws the line the same way: the scoped read runs first and answers 404 for a project this caller cannot see, then ObProjectReadOnlyException answers 403 for the two roles that can see it and may not change it, OB Viewer and OB Step Owner. Those two reached the PATCH or the DELETE because listObProjects and getObProject had already rendered the row on their grid, so a 404 here would deny a project they are looking at; the capability is decided from the module role (ObClientScope.mayWrite) before anything about this project is read. The DELETE's other refusal is NOT here: whether anything points at the project (nine FK tables, two hash-chained) is 409 ob-project-in-use, because that depends on the rows and not on the caller",
