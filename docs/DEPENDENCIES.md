...
| 24 | **C-027** attachment limits endpoint ✅ | Divyansh | **Nothing edits them yet.** `PUT /attachments/limits` exists and is `master.write`; §11's sidebar has a Settings entry and **no S-number specifies an org-wide settings screen**, so an Admin can set the caps by request and not from a page. Whoever owns that screen consumes the endpoint — masters' surface by shape, C's by feature | Ayush? | **Needs a screen, or a decision that the API is enough** |
