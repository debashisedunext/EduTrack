...
| 22 | **B-021** priority master ✅ | Ayush | **S-12's "Admin can add levels" is not deliverable alone.** Opening the contract's `Level` enum retypes `Ticket.level`, `Ticket.originalLevel`, `TaskType.defaultLevel`, `SlaPolicyWrite.level`, `SlaPolicyCell.level`, `ChangeTicketPriorityBody.level` and two query params, and needs C's `LevelPicker.tsx` and `columns.tsx` `Record<Level, …>` chip maps made total. A fifth level is refused at the API until then | Debashis (contract), Divyansh (chip maps), Shivendra (ticket schemas) | **Needs a coordinated A/C/D task** |
