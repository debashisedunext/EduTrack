...
| 23 | **B-021** `listPriorities` is active-only by default ✅ | Ayush | `CreateTicketPage` and `TicketListPage` map the response straight into their pickers **without filtering `isActive`** — the task-type filter one line above has no priority equivalent. The endpoint defaults to active rows so a retired level cannot reach them; once C adds the filter, the default can widen to match `listTaskTypes` and `listModules` | Divyansh | **Needs a small C task** |
