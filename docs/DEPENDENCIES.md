...
| 20 | **`worker/stats/` ownership** — sign-off, not code | Debashis | A-058, A-059 and every later widget needing a new aggregate | Shivendra | **Before A-058** |

> **On #20.** This is the only row here that blocks nobody's keyboard and still belongs in the register, because the thing being waited on is a decision rather than an artifact.
>
> `worker/stats/` is Stream D's path by TEAM-PLAN §6 and Stream A's code by A-051 — it exists only to fill A-050's summary tables, because CLAUDE.md forbids a live `COUNT(*)` behind a dashboard. A-056 and A-057 have edited it three times (`type_counts`, `assigned_in_progress`, `sla_closed`/`sla_met`), each flagged in the code and in a pull request. TEAM-PLAN §6 now carries the proposed carve-out and the reasoning; `.github/CODEOWNERS` is deliberately unchanged, so Debashis is still auto-requested on every one of those pull requests — which is where the answer is most likely to arrive.
>
> **Either answer works.** If the carve-out is accepted, A-058's aggregates are ordinary Stream A work. If Debashis would rather own the stats refresh, they become requests raised against Stream D, and A-058 gains a dependency it does not have today. What does not work is leaving it: A-058's stage funnel and A-059's client volume both need columns that do not exist yet, so the next unannounced edit is already scheduled.
...
| 21 | **B-019** project settings endpoint ✅ | Ayush | **Closed by C-071 ✅.** `ProjectSettingsGate` refuses a task type outside the allow-list and any empty `mandatoryFields` on `POST /tickets`; `CreateTicketPage` filters the picker on `restrictsTaskTypes` + `taskTypes[].isAllowed` and marks the required controls. Both halves in `api/feature/tickets` and `CreateTicketPage` as this row asked | Divyansh | **Resolved** |
| 22 | **B-021** priority master ✅ | Ayush | ✅ **Closed by D-066, 20 Aug.** `Level` is an open string in the contract now, so S-12's "Admin can add levels" works end to end. It turned out to be one stream's change, not three: the backend was **already** open (`UnknownLevelException` validates against the master and its javadoc predicted this; `level` is `@NotBlank String` on every write DTO), so Shivendra's half was empty. Divyansh's half was nearly empty too — `Chip`'s `defaultVariants: { variant: 'neutral' }` already renders an unknown level as a neutral chip carrying its own code, so only the two `LEVEL_LABEL` maps genuinely blanked, and they are now one shared `levelLabel()`. What actually gated it was **two `CONTRACT_LEVELS` allow-lists in the masters services**, which refused a fifth level at the API with a message naming this row; both are deleted, and `PriorityService` now validates the code's *shape* instead | Debashis (contract, mock, chip maps), Ayush (sign-off on the two masters services) | **Done — D-066** |
| 23 | **B-021** `listPriorities` is active-only by default ✅ | Ayush | ✅ **C's half closed by C-072, 21 Aug.** `selectableLevels` in `features/tickets/levels.ts` filters `isActive !== false`, and all **three** call sites go through it — `CreateTicketPage`, `TicketListPage` (the filter *and* the bulk change-level dialog) and `TicketLevelControl`, which this row missed and is the one that actually applies a level. Filtered in the client rather than by the query parameter, so the screens are correct under either default: **`listPriorities` can now widen to match `listTaskTypes` and `listModules` as a Stream B change on its own, with nothing to coordinate.** `TicketLevelControl` keeps a ticket's *current* level even once retired, so retiring CRITICAL does not open S-20's dialog with nothing checked | Divyansh (done), Ayush (the widening, whenever it suits) | **C done — B may widen at will** |
| 24 | **C-027** attachment limits endpoint ✅ | Divyansh | ✅ **Closed by B-068, 22 Aug — decided the API is enough.** `PUT /attachments/limits` stays the only way an Admin sets the caps; no screen is built. Attachment limits are the sole org-wide setting the product has today, and C-027's endpoint already covers it with `master.write` gating and `If-Match`/ETag concurrency — a page around one three-field form would earn nothing the documented `PUT` doesn't already give. `/settings`'s placeholder now says this instead of promising a screen a later task would build. Revisit if a second org-wide setting appears; one setting doesn't justify a surface | Ayush | **Resolved — no screen** |
