...
**Terminology (v1.2 renames, by product decision):** the journey-template page is called **Module Service**; a step's checklist (formerly sub-categories) is its **Task List**; the delivery person owning a service is an **Implementor**. **TATs are defined and displayed in working days**, not hours.

**Deviation, 11 Sep 2026 — a Module Service has four levels, not three.** This line used to say "a journey's steps are **Services**", and that was a rename over an unchanged model. It is now a change to the model: **Module Service → Implementation Stage → Task → Task List**. The Implementation Stage (OB-15) became a **group** that carries nothing — no TAT, no owner, no sign-off — and the **Task** inside it carries all three. `V20260911_1630` adds `ob_journey_template_stages` and re-points each step at one.

**The new level went *above* the existing row rather than in its place, and that was forced rather than chosen.** Ten tables hold a foreign key to `ob_journey_steps` — the clock, the history, sign-offs, escalations, attachments, notifications, the outbox, comms and the step items. That row is the executable unit: it has the status, the owner and the due date. Read the other way round — existing row becomes the group — all ten would anchor to a container with none of those. So a **Task is the row that was a step**, keeping every column, and the stage group is new. No data moved between tables and no running journey was touched.

---

## 1. What we are building

When a client is signed they are boarded through **one journey per Module Service of every product they bought** — EduTrack ERP runs Standard SaaS Onboarding *and* Enterprise (with data migration audit), Biometric Attendance runs its own rollout, and each of those is instantiated from its own admin-defined template with its own steps, sub-categories, owners and TATs. Before any journey starts, the client must clear a set of **prerequisites** — their own responsibilities. Management sees every client, every journey, and where things are stuck; the client sees their own progress through a **portal login**. When every journey completes (final sign-offs included), the client turns **Green — Live**.
...
- **Products master + per-service journey templates**: Admin creates a journey template *for a Module Service of a product* (steps, order, TAT, default owner, sub-categories, sign-off flags); creating a new journey = creating a new service under a product. **A product publishes any number of services at once**, each versioning independently — one active version *per service*, and instances pin their version.

  > **Amended 10 Sep 2026, superseding “one active template version per product”.** That wording made a product and a journey the same thing, so EduTrack ERP could carry exactly one ribbon — and publishing a second service silently retired the first, quietly changing what every client boarded afterwards received. `V20260909_1900` had already made the service *name* its identity and moved version numbering to `(product, name, version)`; `V20260910_0030` finishes the job, moving the active key to `(product, name, active)` and the instance-side key to `(client, product, service, live)`. Every existing template stayed the single service of its product, so no in-flight journey changed.
- **Multiple journeys per client**: each purchased product instantiates one journey per service it publishes. Client page shows journeys as **accordions** — each strip carries product name, % complete, and every step as a **RAG-patterned dot**; expanding shows the full ribbon and step panel.
...
| 4 | **Backup owner per step** | Leave coverage; working-calendar aware. **On the live journey step only, as of `V20260914_1830`** — the *template's* copy is dropped. Who covers for somebody depends on who is on leave in March, and a template has no March. |
...
- `ob_journey_templates` — **+ `product_id NOT NULL`, `sequence` (the service order — drives instantiation and display order for every client), and its cross-service dependency** (cycle-free — the picker excludes anything that transitively depends on it). **Deviation, 11 Sep 2026: a service depends on *several* other module services, not one.** The plan said `depends_on_template_id NULL`, one id on the template row; `V20260911_1100` replaces it with `ob_journey_template_dependencies`, a row per edge, and OB-07's "Depends on" column is a multi-select. The rule itself is unchanged — cross-product, cycle-free, enforced in the service layer — only its arity. `ob_journeys.held_by_journey_id` stays one column and becomes a cursor over the set: the release re-points it at the next outstanding holder and clears it only when none is left. One *active* version per product; creating a new journey = a new template row for a product. / `ob_journey_template_steps` — **+ `depends_on_step_id NULL`**: each step declares at most one dependency, constrained to an **earlier step in the same template** (cycle-free by construction; enforced in the designer and by a service check). NULL = no dependency — the step activates at journey start and **runs in parallel**. / `ob_journey_template_stages` — **new, 11 Sep 2026**: the stage group, one row per implementation stage per template, seeded from the OB-15 master the moment a Module Service is created and **empty** — a stage names a phase, not something to do. `implementation_stage_id` is nullable for the single `Ungrouped` group that collects tasks written before the stages existed; those were deliberately not matched into the vocabulary by name, since a wrong guess silently re-labels a published template. / `ob_journey_template_steps` — **+ `template_stage_id NOT NULL`**, the group the task sits in; `implementation_stage_id` and `uq_ob_journey_template_steps_stage` (one step per stage) are dropped by the same migration, the latter because a stage now holds *many* tasks. `sequence` stays template-wide and is renumbered so ascending order walks groups in order then tasks within a group — which is what lets instantiation, the parallel-group layering, the revision clone and the designer tree all keep sorting by one column. / `ob_journey_template_step_items` (sub-categories) — the **Task List**, now hanging off a task. All versioned.
...
2. **Instantiation at boarding, LOCKED.** The wizard's product multi-select creates one journey per product with `gate_status = LOCKED`: fully visible (dots, owners, TATs) but no step activates *by itself* and no clock starts *by itself*. Everyone sees the plan from day one with zero false breaches. See the deviation under item 3 for what a `LOCKED` journey now permits its owners to do deliberately.
3. **The prerequisite gate.** One service method — *every mandatory task VERIFIED, every non-mandatory task VERIFIED or SKIPPED* — evaluated on every prerequisite transition. When satisfied: all LOCKED journeys flip OPEN, first steps activate, clocks start, kickoff automation fires. Products bought after gate-open instantiate directly OPEN.

   > **Deviation — the gate is advisory, not blocking (agreed with operations, Sep 2026).** As written above and in item 2, `LOCKED` refused *every* path into `IN_PROGRESS`, so one unverified document stopped all implementation work for that client. It now governs only the **automatic** half:
   >
   > - A `LOCKED` journey still activates nothing by itself and starts no clock by itself — item 2's "zero false breaches" is intact, because no TAT accrues that nobody chose to start.
   > - **A step owner may start their own step on a `LOCKED` journey.** `ObJourneyStepLifecycleService#start` no longer reads `gate_status`, and step activation no longer re-asserts it (a chain whose first step was started by hand has to hand on to its second). `journey-not-open` therefore has one cause now — §5.5's service-level hold, which waits on work nobody on that journey can do — and that one is unchanged.
   > - Everything the screens say about the gate is unchanged: the chip, the banner, the checklist and the `LOCKED` filter all still report it, and the step panel notes an uncleared gate beside a live Start.
   >
   > **There is still no "open gate anyway" override** — nothing flips `gate_status` but the evaluation above, and the only valve into it remains skipping non-mandatory tasks. What changed is what a locked gate *prevents*, not who can unlock it.
...
| OB-07 | **Module Service** (journey template designer) — **with a product filter (select a product to see only its services), ↑/↓ card re-sequencing, and a per-service "Service depends on" picker (another service or none/parallel, cross-product allowed, cycles excluded)** | **grouped by product; "+ Create journey template" (name + product) is the way a new journey is defined; every template card shows its total TAT at the top, and the designer header shows the running total while editing — **the critical path through the tasks, not Σ of their TATs**: tasks that depend on nothing run alongside each other, so 1 day beside 2 days is 2 days, and 1 day followed by 2 days is 3; tasks grouped under their implementation stage, with "+ Add a task" per stage and **no way to add or delete a stage** (OB-15 decides those); an empty stage still draws, so a new service shows all six; per-task **Dependency** selector (which may name a task in another stage — drawn as a caption rather than an indent, since a row cannot nest under a parent in another group) (an earlier step, or "none — runs parallel"; reorder/delete re-validates dependencies); per-task **Implementor** and nothing else — no owning role and no backup owner: a task pins nobody by default and takes the *project’s* implementor when a journey is created from it, which is the level where "who, for this client" is known; one active version per product; versioned publish** |
