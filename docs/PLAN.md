...
| D-17 | §5.8's second half: "a service completes only when every sub-category is answered, **and False requires a remark**" — held in the schema by `ck_ob_journey_step_items_remark` (`answer IS NULL OR answer = 1 OR (remark IS NOT NULL AND remark <> '')`), refused ahead of it by `ObJourneyStepLifecycleService#answerItem` and surfaced as the problem type `ob-step-item-remark-required` | **The remark is optional on both answers.** `V20260916_1520__ob_step_item_remark_optional.sql` drops the constraint; the service refusal, the exception class and the problem type go with it. Blank still stores as `NULL` rather than `''`, so `remark IS NOT NULL` stays a question about whether a reason was given. **The rest of §5.8 is untouched** — every mandatory item must still be answered before a service completes, and an item answered False still satisfies that gate exactly as a True does, which is the half `requireCompletionGate` actually enforces | Requested 16 Sep 2026 by Shivendra, with the Check List grid and the single flip-status control that replaced the Verified / Unverified pair. The rule was written for a reader downstream — a False whose reason reads *(none given)* gets actioned by going and asking the person who recorded it — but a constraint cannot tell a considered reason from a cleared box, so in practice it charged every Not completed the same sentence and got whatever satisfied it. **Two costs, stated:** a Not completed may now carry no reason at all, and the only thing left encouraging one is the remark field sitting in the next column — the seed corpus still writes reasons, deliberately, because a demo corpus exists to show the screen at its most readable. And it is **one-way in the sense that matters**: rows written after this migration may hold `answer = 0` with a `NULL` remark, so restoring §5.8 later means backfilling those rows before the constraint can go back. No existing row violated it and no recorded remark is touched |
