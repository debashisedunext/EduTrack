...
| | Tasks | Effort (days) |
|---|---:|---:|
| Complete | 102 of 231 (44%) | 144.0 of 348.8 (41%) |
| In flight | 2 | 2.0 |
| On the driving chain | 50 | 71.0 |
| Zero float (no slack at all) | 61 | 97.0 |

### By developer

| Stream | Developer | Tasks | Done | Effort | Working days available | Load | Finishes |
|---|---|---:|---:|---:|---:|---:|---|
| **A** | Shivendra | 64 | 29 | 93.0 | 78 | 119% ⚠️ | Sat 07 Nov |
| **B** | Ayush | 51 | 17 | 84.8 | 78 | 109% ⚠️ | Tue 03 Nov |
| **C** | Divyansh | 67 | 17 | 99.5 | 78 | 128% ⚠️ | Thu 19 Nov |
| **D** | Debashis | 49 | 39 | 71.5 | 78 | 92% | Sat 07 Nov |
...
| `A-074` Security | Shivendra | Thu 24 Sep | Wed 21 Oct | +19d |
| `A-071` Audit Log Viewer | Shivendra | Tue 22 Sep | Wed 14 Oct | +16d |
| `B-031` Step 1 — template download | Ayush | Tue 25 Aug | Fri 11 Sep | +13d |
| `B-032` Step 2 — upload, max 5 MB / 5,000 rows, event-dr | Ayush | Thu 27 Aug | Tue 15 Sep | +13d |
...
gantt
    ...
    M1 — Authentication & the scope guard — weeks :active, a1, 2026-08-07, 8d
    Hardening — weeks 17–18 :active, a2, 2026-08-12, 53d
    M2 — Immutability core — weeks 8–9 :a3, 2026-08-18, 11d
    M6 — Dashboard & reports — weeks 10–16 :a4, 2026-09-01, 50d
```
...
gantt
    ...
    M3 — Master data — weeks 3–9 :active, b1, 2026-08-10, -17d
    Weeks 12–14 — M6 reports :b2, 2026-10-06, 8d
    Weeks 10–11 — join Stream C on the ribbon :b3, 2026-10-16, 13d
```
...
gantt
    ...
    M5 — SLA  escalation & mail — weeks 6–11 :active, d2, 2026-08-07, 25d
...
|  | `A-035` | Out-of-scope IDs return 404, not 403, on /tickets/{id} and every… | 0.5 | `A-034` | Thu 13 Aug | Thu 13 Aug | 0 | 🟡 50% |
| 🔴 | `A-036` | Permission test matrix | 2 | `A-035` | Thu 13 Aug | Sat 15 Aug | 0 | ▫️ to do |
|  | `A-037` | ArchUnit rules | 1 | `A-034` ᶦ | Sat 15 Aug | Tue 18 Aug | 0 | ▫️ to do |
|  | `A-040` | Append-only services for the three protected tables | 1.5 | `A-010` `A-013` | Tue 18 Aug | Wed 19 Aug | 0 | ▫️ to do |
|  | `A-041` | Canonical JSON serialiser — fixed key order, fixed timestamp format | 1.5 | `A-040` | Thu 20 Aug | Fri 21 Aug | 0 | ▫️ to do |
| 🔴 | `A-042` | Per-ticket hash chain with SELECT … FOR UPDATE on the ticket row… | 2.5 | `A-041` | Fri 21 Aug | Tue 25 Aug | 1 | ▫️ to do |
|  | `A-043` | Compensating-entry pattern — is_correction, corrects_entry_id | 1 | `A-042` | Wed 26 Aug | Wed 26 Aug | 1 | ▫️ to do |
|  | `A-044` | Nightly chain verifier in worker, admin alert on break… | 2 | `A-042` | Thu 27 Aug | Fri 28 Aug | 1 | ▫️ to do |
|  | `A-045` | Concurrency test | 1.5 | `A-042` | Sat 29 Aug | Tue 01 Sep | 1 | ▫️ to do |
|  | `A-050` | daily_ticket_stats and resource_daily_stats summary tables | 1.5 | `A-034` `B-007` ᶦ | Tue 01 Sep | Wed 02 Sep | 2 | ▫️ to do |
|  | `A-051` | 5-minute refresh worker. Dashboard reads never issue live COUNT() | 1 | `A-050` | Thu 03 Sep | Thu 03 Sep | 2 | ▫️ to do |
|  | `A-052` | /tickets/{id}/full aggregated endpoint | 1 | `A-034` ᶦ | Fri 04 Sep | Fri 04 Sep | 2 | ▫️ to do |
|  | `A-053` | Cursor pagination + virtualised grid rendering beyond 200 rows | 1.5 | `A-052` `C-003` ᶦ | Sat 05 Sep | Tue 08 Sep | 2 | ▫️ to do |
|  | `A-054` | Shell, role-aware, with project/date/resource filters | 1.5 | `A-051` `C-005` ᶦ | Tue 08 Sep | Wed 09 Sep | 3 | ▫️ to do |
|  | `A-055` | Widgets 1–6 — KPI cards with sparklines and animated count-up | 2 | `A-054` ᶦ | Thu 10 Sep | Fri 11 Sep | 3 | ▫️ to do |
|  | `A-056` | Widgets 7–12 | 3 | `A-055` ᶦ | Sat 12 Sep | Wed 16 Sep | 3 | ▫️ to do |
|  | `A-057` | Widgets 13–15 — calendar heatmap, SLA radial gauge, project treemap | 2 | `A-056` ᶦ | Thu 17 Sep | Fri 18 Sep | 3 | ▫️ to do |
|  | `A-058` | Widgets 16–19 — stage funnel, rework/ping-pong, avg time per stage | 2.5 | `A-056` `C-049` | Thu 29 Oct | Sat 31 Oct | 10 | ▫️ to do |
|  | `A-059` | Widget 20 — client-wise volume. Depends on Stream B's client master | 1 | `A-056` `B-029` | Wed 28 Oct | Wed 28 Oct | 10 | ▫️ to do |
|  | `A-060` | Every card and chart segment deep-links to a pre-filtered list | 1.5 | `A-056` `C-014` ᶦ | Sat 19 Sep | Tue 22 Sep | 3 | ▫️ to do |
|  | `A-061` | Drill-down modal, slides from the right, CSV export | 1.5 | `A-060` ᶦ | Tue 22 Sep | Wed 23 Sep | 4 | ▫️ to do |
|  | `A-062` | Developer dashboard variant | 1.5 | `A-055` ᶦ | Thu 24 Sep | Fri 25 Sep | 4 | ▫️ to do |
|  | `A-063` | Reports hub | 2 | `A-051` `C-003` ᶦ | Fri 25 Sep | Tue 29 Sep | 5 | ▫️ to do |
|  | `A-064` | Export engine — Excel, CSV, PDF | 2 | `A-063` ᶦ | Tue 29 Sep | Thu 01 Oct | 6 | ▫️ to do |
|  | `A-065` | Scheduled report email (daily/weekly/monthly) | 1 | `A-064` `D-029` | Tue 27 Oct | Tue 27 Oct | 10 | ▫️ to do |
|  | `A-066` | Reports 1–6 | 3 | `A-063` ᶦ | Thu 01 Oct | Tue 06 Oct | 7 | ▫️ to do |
|  | `A-067` | Reports 7–12 | 3 | `A-066` ᶦ | Tue 06 Oct | Fri 09 Oct | 8 | ▫️ to do |
|  | `A-068` | Reports 13–18 | 3 | `A-067` `C-058` | Thu 05 Nov | Sat 07 Nov | 8 | ▫️ to do |
|  | `A-069` | Resource 360° profile | 1.5 | `A-066` `B-010` | Fri 09 Oct | Sat 10 Oct | 9 | ▫️ to do |
|  | `A-070` | "Born critical vs became critical" report | 1 | `A-066` `D-028` | Sat 24 Oct | Sat 24 Oct | 10 | ▫️ to do |
|  | `A-071` | Audit Log Viewer | 1.5 | `A-034` ᶦ | Tue 13 Oct | Wed 14 Oct | 9 | ▫️ to do |
|  | `A-072` | Global search + ticket-ID deep link | 1.5 | `A-009` `C-005` ᶦ | Wed 14 Oct | Thu 15 Oct | 10 | ▫️ to do |
|  | `A-073` | Performance | 2 | `A-053` `B-007` ᶦ | Fri 16 Oct | Sat 17 Oct | 10 | ▫️ to do |
|  | `A-074` | Security | 2 | `A-036` ᶦ | Tue 20 Oct | Wed 21 Oct | 10 | ▫️ to do |
|  | `A-075` | Go-live runbook, deployment, TLS, secrets in vault | 2 | `A-073` `A-074` ᶦ | Thu 22 Oct | Fri 23 Oct | 10 | ▫️ to do |
...
|  | `B-015` | Role & permission master — module × CRUD/approve checkbox matrix | 2 | `B-001` `C-003` ᶦ | Thu 13 Aug | Thu 13 Aug | 70 | ✅ done |
|  | `B-016` | Project master list/create/edit — code | 2 | `B-005` `C-003` ᶦ | Thu 13 Aug | Fri 14 Aug | 0 | ▫️ to do |
|  | `B-017` | Team tab — resources + per-project role | 1.5 | `B-016` ᶦ | Sat 15 Aug | Tue 18 Aug | 0 | ▫️ to do |
|  | `B-018` | SLA tab | 1.5 | `B-016` ᶦ | Tue 18 Aug | Wed 19 Aug | 0 | ▫️ to do |
|  | `B-019` | Settings tab | 1.5 | `B-016` ᶦ | Thu 20 Aug | Fri 21 Aug | 0 | ▫️ to do |
|  | `B-020` | Task type master — the 11 seeded types, Admin-extensible | 1.5 | `B-002` `C-003` ᶦ | Fri 21 Aug | Sat 22 Aug | 0 | ▫️ to do |
|  | `B-021` | Priority master | 1.5 | `B-002` `C-003` | Tue 25 Aug | Wed 26 Aug | 0 | ▫️ to do |
|  | `B-022` | Notification template master | 2 | `B-005` `C-003` | Wed 26 Aug | Fri 28 Aug | 0 | ▫️ to do |
...
|  | `B-025` | Client list | 2 | `B-005` `C-003` ᶦ | Fri 28 Aug | Tue 01 Sep | 0 | ▫️ to do |
|  | `B-026` | Client create/edit across four tabs | 3 | `B-025` ᶦ | Tue 01 Sep | Fri 04 Sep | 0 | ▫️ to do |
|  | `B-027` | client_contacts child grid | 1.5 | `B-026` ᶦ | Fri 04 Sep | Sat 05 Sep | 0 | ▫️ to do |
|  | `B-028` | Validation | 1 | `B-027` | Tue 08 Sep | Tue 08 Sep | 0 | ▫️ to do |
|  | `B-029` | Deactivating a client with open tickets warns and blocks new… | 1 | `B-026` ᶦ | Wed 09 Sep | Wed 09 Sep | 0 | ▫️ to do |
| 🔴 | `B-030` | Import engine as a schema registry — built once, registered twice | 2 | `B-005` | Tue 11 Aug | Tue 11 Aug | 12 | ✅ done |
|  | `B-031` | Step 1 — template download | 1.5 | `B-030` ᶦ | Thu 10 Sep | Fri 11 Sep | 0 | ▫️ to do |
|  | `B-032` | Step 2 — upload, max 5 MB / 5,000 rows, event-driven SAX parse | 2 | `B-030` ᶦ | Fri 11 Sep | Tue 15 Sep | 0 | ▫️ to do |
|  | `B-033` | Step 3 | 2 | `B-032` ᶦ | Tue 15 Sep | Thu 17 Sep | 0 | ▫️ to do |
| 🔴 | `B-034` | Step 4 — dry-run validation preview | 2.5 | `B-033` | Thu 17 Sep | Sat 19 Sep | 0 | ▫️ to do |
|  | `B-035` | Step 5 — commit as a background job with progress bar | 2 | `B-034` | Tue 22 Sep | Wed 23 Sep | 0 | ▫️ to do |
|  | `B-036` | Error report generation | 1 | `B-034` ᶦ | Thu 24 Sep | Thu 24 Sep | 0 | ▫️ to do |
|  | `B-037` | import_batches traceability | 1 | `B-035` ᶦ | Fri 25 Sep | Fri 25 Sep | 0 | ▫️ to do |
|  | `B-038` | Resource bulk import — the second registration, not a second build | 1 | `B-035` | Sat 26 Sep | Sat 26 Sep | 0 | ▫️ to do |
|  | `B-039` | Status/stage/workflow master tab 1 | 2 | `B-003` `C-003` ᶦ | Tue 29 Sep | Wed 30 Sep | 0 | ▫️ to do |
|  | `B-040` | Tab 2 — stages | 2 | `B-039` | Thu 01 Oct | Fri 02 Oct | 0 | ▫️ to do |
|  | `B-041` | Tab 3 | 2.5 | `B-040` `B-050` | Tue 20 Oct | Thu 22 Oct | 11 | ▫️ to do |
| 🔴 | `B-042` | Stages in use may be deprecated, never deleted | 1 | `B-040` | Sat 03 Oct | Sat 03 Oct | 24 | ▫️ to do |
|  | `B-043` | Workflow template designer | 3 | `B-041` `B-042` | Fri 23 Oct | Tue 27 Oct | 11 | ▫️ to do |
|  | `B-050` | Ribbon segment component — 6 states | 2.5 | `C-003` `C-042` | Fri 16 Oct | Tue 20 Oct | 10 | ▫️ to do |
|  | `B-051` | Compact dot variant for the ticket list | 1 | `B-050` ᶦ | Wed 28 Oct | Wed 28 Oct | 11 | ▫️ to do |
|  | `B-052` | Ribbon accessibility | 1.5 | `B-050` ᶦ | Thu 29 Oct | Fri 30 Oct | 11 | ▫️ to do |
|  | `B-053` | Readability at 8 stages on a laptop | 2 | `B-050` ᶦ | Fri 30 Oct | Tue 03 Nov | 12 | ▫️ to do |
|  | `B-060` | Client report | 2 | `A-064` `B-029` | Tue 06 Oct | Thu 08 Oct | 8 | ▫️ to do |
|  | `B-061` | Resource performance scorecard and workload/capacity report | 2 | `A-064` `B-010` | Thu 08 Oct | Sat 10 Oct | 9 | ▫️ to do |
|  | `B-062` | Export engine integration for all report types | 1.5 | `A-064` | Sat 10 Oct | Tue 13 Oct | 10 | ▫️ to do |
|  | `B-063` | Timesheet view — stage-aware, a resource's week across all tickets | 2 | `A-064` `C-061` | Wed 14 Oct | Thu 15 Oct | 10 | ▫️ to do |
| 🔴 | `B-064` | Module master read endpoint | 0.25 | `C-065` | Tue 06 Oct | Tue 06 Oct | 29 | ▫️ to do |
...
|  | `D-028` | original_level preserved so "born critical vs became critical"… | 1 | `D-020` ᶦ | Thu 13 Aug | Fri 14 Aug | 54 | ▫️ to do |
|  | `D-029` | Thymeleaf templates driven by Stream B's notification template… | 1.5 | `D-010` `B-022` | Sat 29 Aug | Tue 01 Sep | 46 | ▫️ to do |
|  | `D-030` | Mail body | 1.5 | `D-029` ᶦ | Tue 01 Sep | Wed 02 Sep | 47 | ▫️ to do |
...
|  | `D-037` | All 15 mail events from §4B.6 wired | 2 | `D-030` `D-036` ᶦ | Thu 03 Sep | Fri 04 Sep | 47 | ▫️ to do |
|  | `D-038` | Daily digest 08:30 and weekly manager summary | 1.5 | `D-037` ᶦ | Sat 05 Sep | Tue 08 Sep | 47 | ▫️ to do |
|  | `D-039` | Inbound webhook — reply-to-comment parsing with quoted text stripped | 2 | `D-032` ᶦ | Wed 12 Aug | Wed 12 Aug | 71 | ✅ done |
|  | `D-040` | All 24 events from blueprint §11 across in-app / bell / email… | 2 | `D-012` `B-022` | Tue 08 Sep | Thu 10 Sep | 48 | ▫️ to do |
...
|  | `D-053` | File and image share, emoji, message search | 2 | `D-050` ᶦ | Fri 14 Aug | Sat 15 Aug | 55 | ⛔ blocked |
|  | `D-054` | TKT-xxxx link preview rendering as a rich ticket card | 1 | `D-050` ᶦ | Thu 13 Aug | Thu 13 Aug | 70 | ✅ done |
