...
/**
 * B-122 · OB-10's exports, on the viewer.
 *
 * <h2>Why this is a fetch and not a link</h2>
 *
 * The obvious implementation is
 * `<a href="/api/v1/onboarding/reports/journey-funnel?export=xlsx" download>`,
 * and it does not work: the access token lives in memory and is attached by
 * `http()`, so a browser-initiated navigation carries no `Authorization` header
 * and the server answers 401. The failure is the confusing kind — the browser
 * saves the 401 body as a file called `journey-funnel-2026-09-06.xlsx`, so the
 * user gets a download that opens as gibberish rather than an error they can
 * read.
 *
 ...
 *
 * <h2>Two formats, and PDF is deliberately not one</h2>
 *
 * The shared engine writes PDFs and this module refuses to. A-118's reasoning
 * is on the contract: the module's only PDF is B-116's archived sign-off
 * certificate, which is a legal record generated once and stored, not a
 * rendering of a filtered grid. A second kind of onboarding PDF in circulation
 * would be indistinguishable at a glance from the one that is evidence.
 *
 * <h2>The filename comes from the server</h2>
 *
 * `Content-Disposition` already carries the dated name, built by
 * `ExportDelivery.filenameFor`. Re-deriving it here would be a second naming
 * rule to keep in step, and the one that drifted would be the one nobody
  ...
  async function download(format: string) {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE}/onboarding/reports/${encodeURIComponent(reportKey)}?${query}`,
        { headers: token ? { Authorization: `Bearer ${token}` } : {} },
      )

      if (!response.ok) {
        // Deliberately not saved to disk. A 400 or 500 body written out as an
        // .xlsx is the failure this whole component exists to avoid.
...
/**
 * Reads the server's filename, falling back to the same shape when the header
 * is missing — which it is under a proxy that strips it, and in a test.
 */
